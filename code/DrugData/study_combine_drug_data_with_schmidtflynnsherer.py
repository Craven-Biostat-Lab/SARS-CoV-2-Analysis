# formats data with drugs so study can be used as the target node
...
with open('study_schmidtflynnsherer_drug_protein_rows_v1.csv', 'w', newline='') as file:
    ...
    for item in AllData:
        for prot in DrugProteins:
            if item['symbol_id'] == prot['symbol_id']:
                ...
                writer.writerow((prot['drug_name'],item["unique_protein_id"],item["species"],item["symbol_id"],"-","-",item["symbol_id"],"Protien Drug",prot["approved"]))
